...
const fs = require('fs');
const { runElevatedPowerShellScript, SCRIPTS_DIR } = require('./driver-manager');
 ...
 *  Shows a real UAC prompt via sudo-prompt - this driver requires
 *  elevation for every action, verified directly. */
async function enableVirtualDisplay({ width, height } = {}) {
  ...
  await runElevatedPowerShellScript(MANAGER_SCRIPT, ['-Action', 'enable']);

  if (width && height && scriptExists(CHANGERES_SCRIPT)) {
    // Best-effort - resolution change failing shouldn't block Extend Mode.
    try {
      await runElevatedPowerShellScript(CHANGERES_SCRIPT, ['-xres', String(width), '-yres', String(height)]);
    } catch (_) {
      /* non-fatal */
    }
  }
...
async function disableVirtualDisplay() {
  ...
  await runElevatedPowerShellScript(MANAGER_SCRIPT, ['-Action', 'disable']);
}
