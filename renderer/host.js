...
async function refreshExtendStatus() {
  ...
  // NOTE: this driver's own "status" script doesn't return a reliable
  // installed/enabled flag we can parse (verified against its real
  // output), so we don't gate the Enable button on it. If the driver
  // truly isn't installed, clicking Enable will simply fail with a
  // clear error (no virtual display appeared) - that's the real signal.
  el.extendSetupWarning.style.display = 'none';
  el.extendSetupBtn.textContent = '⚙ Install / Repair Driver (Admin)';
...
async function runExtendSetupStep() {
  ...
  try {
    ...
    showToast('Driver install step ran — a UAC/PowerShell window may have appeared.', 'success');
  } catch (err) {
...
async function toggleExtendMode() {
  ...
  try {
    if (!state.extend.active) {
      showToast('Enabling Extend Mode… approve any Windows/UAC prompt that appears (this can take up to a minute).');
    ...
    showToast(
      'Extend Mode error: ' + err.message + ' — if this keeps happening, click "Install / Repair Driver" first.',
      'error'
    );
  } finally {
    await refreshExtendStatus();
